...
backup="/root/tech4learn-backups/before-photos-$(date -u +%Y%m%dT%H%M%S).dump"
cd /tmp
...
printf '\nStudent photos deployed. Open a student profile to record consent and add photos. Face matching requires a configured private verification engine.\n'
