...
backup="/root/tech4learn-backups/before-bulk-attendance-$(date -u +%Y%m%dT%H%M%S).dump"
cd /tmp
...
printf '\nMulti-photo attendance deployed. Capture a class photo, add more photos before review, then compare. Matching requires the configured face engine.\n'
