...
backup="/root/tech4learn-backups/before-student-photo-setup-$(date -u +%Y%m%dT%H%M%S).dump"
cd /tmp
runuser -u postgres -- pg_dump --port=5432 --format=custom tech4learn_app > "$backup"
test -s "$backup"
pg_restore --list "$backup" >/dev/null
printf 'Private backup: %s\n' "$backup"
release_backup="${backup%.dump}-files"
mkdir -p "$release_backup"
cp -a "$repo/apps/api/dist" "$release_backup/api"
cp -a "$repo/apps/admin/dist" "$release_backup/admin"
app_stopped=false
recover_release() {
  result=$?
  trap - EXIT
  if [ "$result" -ne 0 ]; then
    cp -a "$release_backup/api/." "$repo/apps/api/dist/"
    cp -a "$release_backup/admin/." "$repo/apps/admin/dist/"
    if [ "$app_stopped" = true ]; then systemctl restart tech4learn; fi
    printf '\nUpdate failed; restored the previous application files. Database backup: %s\n' "$backup" >&2
  fi
  exit "$result"
}
...
app_stopped=false
trap - EXIT
printf '\nUnified student photo setup deployed. Refresh Learners > Photos & attendance.\n'
