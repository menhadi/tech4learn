...
class Tech4LearnStudentController extends Tech4LearnPlatformController
{
 public function attempt(Request $r,string $org,string $action){
  ...
  abort_unless(strlen($r->getContent())<=30000,422);
  ...
  abort_unless(strlen(json_encode($input,JSON_THROW_ON_ERROR))<=30000,422);
  try {
   ...
   if(!in_array($status,[403,404,409,422],true))throw $e;
