...
function denyExtraction(callable $call,int $status){try{$call();throw new RuntimeException('Expected extraction failure');}catch(Symfony\Component\HttpKernel\Exception\HttpExceptionInterface $e){checkExtraction($e->getStatusCode()===$status,'Expected bounded failure status');}}
