...
final class Tech4LearnAnswerUploadCoordinator
{
 public function save(string $workspace,int $source,string $learner,int $attempt,int $question,array $input,UploadedFile $file):array {
  ...
  abort_unless($source>0&&$attempt>0&&$question>0&&array_diff(array_keys($input),['request_id','revision'])===[],422);
  ...
  $fingerprint=hash('sha256',json_encode(['attachment',$source,$learner,$attempt,$question,$revision,$hash],JSON_THROW_ON_ERROR));
  ...
  return app(Tech4LearnStudentContext::class)->run((int)$w->organization_id,$student,['exam_result_id'=>$attempt,'question_id'=>$question],function($request)use($workspace,$source,$learner,$student,$attempt,$question,$requestId,$revision,$fingerprint,$file){
   $request->files->set('answer_file',$file);$stat=null;
   $before=function()use($workspace,$source,$learner,$student,$attempt,$question,$requestId,$revision,$fingerprint,&$stat){
   ...
   $complete=function(JsonResponse $response)use($workspace,$source,$learner,$student,$requestId,$fingerprint,$attempt,$question,&$stat){
    $w=$this->workspace($workspace,$source,true);
    $mapped=DB::table('tech4learn_workspace_users')->where('workspace_id',$workspace)->where('local_id',$learner)->where('kind','student')->value('external_id');
    ...
    $receipt=['success'=>true,'saved'=>true,'attempt_id'=>$attempt,'question_id'=>$question,'asset'=>hash('sha256',$path),'revision'=>app(Tech4LearnAttemptAnswers::class)->revision($fresh)];
