...
try {
 ...
 check($call('review',array_replace($reviewBody,['actor_id'=>$learner]))['error']['status']===404,'Student mapping is not a staff review actor');
 DB::table('organization_users')->where('organization_id',20)->where('user_id',1)->update(['status'=>0]);
 check($call('review',$reviewBody)['error']['status']===403,'Revoked native staff cannot read private attachments');
