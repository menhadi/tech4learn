...
check(count($review['questions'])===1&&$review['questions'][0]['answer_html']==='Synthetic written answer','Pending answer available to scoped marker');
...
$q->question='<svg><text>Unsupported image</text></svg>';$q->save();
$mediaReview=$marking->review($workspace,10,$actor,$newLearner,$manual->id);
check(!$mediaReview['questions'][0]['review_supported'],'Media question flagged for complete review');
...
$q->question='<p>Explain \\(x^2\\)</p><math><mfrac><mn>1</mn><mn>2</mn></mfrac></math>';$q->save();
$formulaReview=$marking->review($workspace,10,$actor,$newLearner,$manual->id);
check($formulaReview['questions'][0]['review_supported']&&$formulaReview['questions'][0]['question_html']===$q->question,'Formatted question preserved for the rich renderer');
$q->question=$originalQuestion;$q->save();
$review=$marking->review($workspace,10,$actor,$newLearner,$manual->id);
$extra=$pending->replicate();$extra->save();
DB::statement('ALTER TABLE passage_langs ADD COLUMN passage TEXT');
$passage=App\Models\Passage::create(['organization_id'=>20,'name'=>'Synthetic passage']);
$passageLang=App\Models\PassageLang::create(['passage_id'=>$passage->id,'language_id'=>$q->language_id,'passage'=>'<p>Read this passage.</p>']);
$q->passage_id=$passage->id;$q->save();
$passageReview=$marking->review($workspace,10,$actor,$newLearner,$manual->id);
check($passageReview['questions'][0]['passage']['html']==='<p>Read this passage.</p>'&&$passageReview['questions'][0]['review_supported'],'Owned passage included for marking');
$passageLang->passage='<p>Passage changed.</p>';$passageLang->save();
rejectAnswer(fn()=>$marking->save($workspace,10,$actor,$newLearner,$manual->id,[$pending->id=>5,$extra->id=>5],$passageReview['revision'],$next()),'Changed passage invalidates marks');
$passage->organization_id=99;$passage->save();
rejectAnswer(fn()=>$marking->review($workspace,10,$actor,$newLearner,$manual->id),'Foreign passage cannot be read');
$passage->organization_id=20;$passage->save();$passageLang->passage='<img src="/private.png">';$passageLang->save();
$reviewSource='data:image/png;base64,'.$png;$reviewAsset=hash('sha256',$reviewSource);
$passageLang->passage='<img src="'.$reviewSource.'">';$passageLang->save();
$imageReview=$marking->review($workspace,10,$actor,$newLearner,$manual->id);
check($imageReview['questions'][0]['review_supported']&&str_contains($imageReview['questions'][0]['passage']['html'],'t4l-media:'.$reviewAsset),'Passage images use private asset references');
$image=$marking->media($workspace,10,$actor,$newLearner,$manual->id,$pending->id,$reviewAsset);
check($image['mime']==='image/png'&&$image['base64']===$png,'Owned passage image readable for submitted pending answer');
rejectAnswer(fn()=>$marking->media($workspace,99,$actor,$newLearner,$manual->id,$pending->id,$reviewAsset),'Foreign image source');
rejectAnswer(fn()=>$marking->media($workspace,10,$actor,$learner,$manual->id,$pending->id,$reviewAsset),'Foreign image learner');
rejectAnswer(fn()=>$marking->media($workspace,10,$actor,$newLearner,$manual->id,$pending->id,str_repeat('f',64)),'Unreferenced asset denied');
