...
Tech4Learn superadmin manages the central ExamElite connection and per-organisation feature restrictions. Native exam capabilities are available by default. Organisation edits create owned copies; only superadmin edits shared originals. The [native workspace](examelite-native-workspace.md) implements these rules locally using ExamElite's own interfaces and engine, with user deployment and live workflow verification still required. Shared content never shares student records or results across organisations. The verified pilot's selected exam and mapped result remain available through the legacy reader.
