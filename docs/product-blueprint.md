...
Tech4Learn superadmin manages the central ExamElite connection and per-organisation feature restrictions. The target is default availability of integrated native capabilities, subject to those controls; unfinished adapters are not working features. Organisation edits create owned copies; only superadmin may edit shared originals. The earlier [native workspace](examelite-native-workspace.md) supplies backend infrastructure, while the [same-domain implementation](examelite-central-content.md) replaces its external interface. Local authoring, student delivery and marking milestones are implemented, but full feature parity and live workflow verification remain unfinished. Shared content never shares student records or results across organisations. The verified pilot's selected exam and mapped result remain available through the legacy reader.
