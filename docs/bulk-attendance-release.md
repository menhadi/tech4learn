...
Starting classroom capture from the browser checks for today’s attendance in the organisation timezone. An existing pending record opens for another camera capture using the extra-photo endpoint; confirmed records open for review. The five-photo limit and immutable evidence remain enforced. This routing does not confirm recognition accuracy or change marks.

Temporary repeat-testing exception: captures created for the stored organisation slug `vector-academy` are independent Test runs, including after a previous confirmation. Migration 12 exempts these server-labelled snapshots from daily uniqueness. They remain scoped, reviewed and bounded by existing photo capacity, and are excluded from confirmed learner totals. Other organisations retain daily uniqueness. Remove the named slug check in AttendanceService.start to end the exception; historical test labels and exclusions must remain.

Pilot test-run review now accepts location warnings without acknowledgement or a location-only reason, and allows the authorised reviewer to review their own test capture. GPS evidence remains stored unchanged. The UI presents these as accepted for testing. Every learner still requires an explicit mark; rejection and correction reasons, tenant permissions and normal non-test policies remain enforced.

Comparison failures now retain safe application-authored reasons, including missing references and access changes. The private verification adapter distinguishes authentication, endpoint, photo rejection, rate limiting, connection/timeout and malformed-response failures without exposing keys or provider response bodies. Existing stored failures remain unchanged; retry creates a job with the improved diagnostic.
