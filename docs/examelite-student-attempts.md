...
## Private staff review API (screen pending)
...
Image responses use no-store and nosniff headers. Each successful image view creates an audit entry containing the actor and scoped record IDs, never image bytes. Metadata projections strip image fields, and expired images are rejected before the hourly deletion task runs. History is paged at 50 attempts; capture metadata is bounded by the 1,200-capture attempt limit. Submitted attempts remain reviewable while evidence is retained. The staff review screen and pre-start camera readiness remain unfinished, so proctored starts are still blocked.
