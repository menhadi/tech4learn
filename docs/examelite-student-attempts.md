...
## Staff result review and text marking
...
This slice supports text review only, including the student's written answer and stored reference. Questions requiring images, passages or formatted mathematics are explicitly blocked from marking by both the screen and native adapter until their complete display is connected. Question edits invalidate the review revision. HTTP tests cover scope, restrictions, malformed marks, conflicts, response minimisation, audit fields and membership revocation during a request. A synthetic browser test verifies result selection, identical lost-response retry with one grading, removal of stale answers and student switching. Rich media marking and broader exam workflow verification remain outstanding.
