...
This slice supports formatted question, answer and reference review through the existing sanitised exam renderer, including tables, MathML and bounded TeX. The shared renderer accepts the native single-dollar inline format as well as parenthesised and display TeX. The marking form waits for every content block to render; a formula error keeps Save disabled. Questions requiring images or passages remain explicitly blocked by both the screen and native adapter until their complete display is connected. Question edits invalidate the review revision. HTTP tests cover scope, restrictions, malformed marks, conflicts, response minimisation, audit fields and membership revocation during a request. Synthetic browser tests verify formula rendering, rendering-failure blocking, identical lost-response retry with one grading, removal of stale answers and student switching. Image/passage marking and broader exam workflow verification remain outstanding.
