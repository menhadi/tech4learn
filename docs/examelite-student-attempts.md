...
The native add-on now has an isolated evidence store and a same-domain student capture API. The camera UI is not enabled yet. The existing proctored-paper start gate remains in place until capture, authorised review and scheduled retention are complete. ExamElite's legacy public-image upload is not used.
