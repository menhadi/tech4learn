...
- Staff with exam management access can edit an existing organisation-owned question inside Tech4Learn: basic formatted text, options, answers, hints, explanations and marks. Changes invoke the installed ExamElite controller with a private mapped staff context. Native validation remains authoritative.
...
New question creation, central-original editing, taxonomy editors, the native media/formula editor, exam builder, publication, student sign-in/attempt/resume/submission, marking, native result screens, plan catalogue and per-provider module availability still require implementation. The current editor supports existing organisation-owned questions only. Feature restrictions for the existing native backend are retained, but are not evidence that each replacement internal screen is ready.
